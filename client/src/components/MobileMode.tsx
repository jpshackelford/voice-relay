...
export function MobileMode({ 
  ...
  //
  // Bug we fixed in #469: this effect's stated intent ("only re-run
  // when inputMode changes") didn't match its dep array, which
  // included `stopListening` and `cleanupAudioStream`. Both were
  // unstable because `useSttEngine` returned a fresh `stopListening`
  // callback every render — `useSttEngine`'s own `stopListening`
  // `useCallback` depended on the `hosted` and `ws` *objects* returned
  // by the inner hooks, and those return a new object literal each
  // render. Net effect: every render with a dep-identity shift
  // re-fired this effect; when `setIsListening(true)` ran inside
  // `onstart`, the re-render triggered the effect, the ref guard saw
  // mic active, and stopListening() got called ~2 ms after onstart —
  // killing STT and producing the iOS Safari `aborted/No speech
  // detected` we'd been chasing for two PRs.
  //
  ...
  //
  // After #471 landed, `useSttEngine`'s `startListening`/`stopListening`
  // callbacks have stable identities across renders, so this
  // `prevInputModeRef` guard is technically redundant. Kept as
  // belt-and-suspenders because the cost is one ref + one equality
  // check, and it documents intent: this effect must only fire on an
  // actual inputMode transition. If a future regression re-introduces
  // identity churn upstream, this guard contains the blast radius
  // until the contract test in `useSttEngine.test.ts` catches it.
